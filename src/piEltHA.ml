...
(* atom type used by the atom pairs molecular encoding
...
type t = { pi: int; (* number of pi electrons *)
           ...
           nbHA: int } (* number of connected heavy atoms (direct neighbors) *)

let dummy = { pi = -1; elt = "KO"; nbHA = -1 }

let to_string x =
  sprintf "%s%s%s"
    (if x.pi = 0 then "" else Utls.string_of_digit x.pi)
    x.elt
    (if x.nbHA = 0 then "" else Utls.string_of_digit x.nbHA)
